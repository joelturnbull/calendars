class LocationsController < ApplicationController
  ...
  def index
    respond_to do |format| 
      ...
      format.ics do
        ...
        Location.master_feed.record_subscription_from_ip(request.remote_ip, params[:subscription_type])
      end
    end
  end
  ...
  def show
    ...
    location.record_subscription_from_ip(request.remote_ip, params[:subscription_type])
  end
  ...
  def google_subscribe_link(location)
    "http://www.google.com/calendar/render?cid=#{CGI.escape(location_url(location, subscription_type: 'google'))}"
  end

  def ical_subscribe_link(location)
   "webcal://#{CGI.escape(location_url(location, subscription_type: 'webcal').gsub("http://",""))}"
  end

  def ics_link(location)
    location_url(location, subscription_type: 'ics')
  end
